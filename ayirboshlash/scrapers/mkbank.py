...
URL = "https://mkbank.uz/uz/services/exchange-rates/"
...
def scrape() -> list[dict]:
    ...
    with sync_playwright() as p:
        ...
        try:
            logger.info("Mikrokreditbank ochilmoqda...")
            page.goto(URL, wait_until="domcontentloaded", timeout=60000)
            page.wait_for_timeout(4000)

            text = page.inner_text("body")

            # Format: USD\t12010\t12090\t12035.22
            pattern = re.compile(
                r'^(USD|EUR|RUB|GBP|CHF|JPY|KZT)\t([\d.]*)\t([\d.]*)\t([\d.]*)$',
                re.MULTILINE
            )
...
if __name__ == "__main__":
    ...
    rates = scrape()
    if rates:
        save_rates("mkbank", rates)
        logger.success("Mikrokreditbank bazaga saqlandi!")
